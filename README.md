...
- Alloc이 거부되면 클라이언트는 기동하지 못하고 3초 간격으로 재시도한다. 헬스체크가 계속 Unhealthy이므로 Id를 발급하지 않으며, NTP가 교정되는 순간 자동으로 정상 기동한다(재시작 불필요). 거부 사유는 클라이언트 로그에 응답 본문 그대로 남는다.
- requester별 마지막 관측값은 Redis에 기록되고(임대 기간 + 유예 시간 후 자동 만료) 관리 화면 `Allocated Ids`의 **Clock Skew** 컬럼에서 볼 수 있다.
  - 최초 Alloc이 거부된 클라이언트는 할당된 Id가 없어 이 화면에 나타나지 않는다. 이 경우는 서버 경고 로그(requester·오차·임계값·remoteIp 포함)로 확인한다.
